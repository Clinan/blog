...
在用拥塞避免的方法探寻更多可用带宽之前的`cwnd`值，以帮助TCP建立`ACK`时钟。
...
快速恢复带来了一个新的问题，当一个传输窗口中出现多个数据包丢失时，一旦其中一个包重传成功，发送方就会接收到一个成功的 `ACK`，这样快速恢复中`cwnd`窗口的暂时膨胀就会停止，而事实上丢失的其他数据包坑你未完成重传。导致出现这种情况的`ACK`成为局部`ACK`（`partial ACK`）。Reno算法在接收到局部`ACK`后就停止拥塞窗口膨胀阶段，并将其减小到特定值。这会严重的浪费网络性能。
