...
- `partition.assignment.strategy`在一个消费者群组里，有一个消费者掉线了，需要将掉线的消费者所消费的分区，交由其他消费者进行消费。这个配置分区分配过程的策略。**这个策略必须是消费者leader的策略**。
  - `Range`
...
- 消费者只能看到已经复制到ISR的数据
  ...
  ![image-20210614132711012](/Users/xli/Library/Application Support/typora-user-images/image-20210614132711012.png)
...
- broker会伟分区里的每个片段打开一个文件句柄，哪怕片段不是活跃片段。这样会导致打开过多的文件句柄。
...
- 为了帮助 broker 更快地定位到指定的偏移量，Kafka 为每个分区维护了一个索引。
